...
class OrderList(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # user_checkout = UserCheckout.objects.get_or_create(id=user_check_id)
        return super(OrderList, self).get_queryset().filter(user__user=user_check_id)
...
class UserAddressCreateView(CreateView):
    ...
    def get_checkout_user(self):
        user_check_id = self.request.session.get("user_checkout_id")
        user_checkout = UserCheckout.objects.get_or_create(id=user_check_id)[0]
...
class AddressSelectFormView(CartOrderMixin, FormView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if b_address.count() == 0:
            ...
            messages.success(self.request, "Please add a shippping address before continuing")
            return redirect("user_address_create")
        else:
